...
const BASE_WEAPONS: Record<string, Weapon> = {
  ...
  havoc: {
    key: "havoc", name: "Havoc", color: "#ff7043", color2: "#ffcbb2", sprite: "fp47",
  ...
  vanguard: {
    key: "vanguard", name: "Vanguard AR", color: "#7bf2a8", color2: "#ccffdd", sprite: "gv_assaultrifle2",
  ...
  wraith: {
    key: "wraith", name: "Wraith SMG", color: "#a0c4ff", color2: "#e2eeff", sprite: "gv_smg2",
  ...
  gatling: {
    key: "gatling", name: "Grave Gatling", color: "#ff6ad5", color2: "#ffc7ee", sprite: "pg_minigun",
  ...
  buzzsaw: {
    key: "buzzsaw", name: "Buzzsaw", color: "#66d9ff", color2: "#d3f4ff", sprite: "pg_subfusile",
    rate: 0.055, damage: 2.0, pellets: 1, spread: 0.1, speed: 1000, bulletRadius: 4,
    pierce: 1, knock: 3, shake: 2, visual: "tracer", rarity: "rare",
  },
  cleaver: {
    key: "cleaver", name: "Bone Cleaver", color: "#f2e8cf", color2: "#ffffff", sprite: "ml5",
    rate: 0.28, damage: 9, pellets: 4, spread: 0.5, speed: 520, bulletRadius: 7,
    pierce: 2, knock: 22, shake: 6, visual: "bolt", rarity: "uncommon",
  },
  warblade: {
    key: "warblade", name: "Warblade", color: "#ff8fab", color2: "#ffd8e2", sprite: "ml20",
    rate: 0.22, damage: 12, pellets: 3, spread: 0.45, speed: 560, bulletRadius: 8,
    pierce: 3, knock: 24, shake: 7, visual: "ring", rarity: "rare",
  },
  hammerfall: {
    key: "hammerfall", name: "Hammerfall", color: "#c0a080", color2: "#f0e0c8", sprite: "ml34",
    rate: 0.75, damage: 26, pellets: 5, spread: 0.6, speed: 480, bulletRadius: 10,
    pierce: 4, knock: 40, shake: 15, visual: "ring", rarity: "legendary",
  },
};

/* ------------------------ procedural pack weapons -------------------------- */
/* Every gun / melee sprite shipped in the art packs becomes a usable weapon. */
...
const MELEE_SUFFIX = ["Edge", "Cleaver", "Fang", "Crusher", "Slicer", "Reaver", "Bite", "Splitter"];
...
function makePackWeapon(sprite: string): Weapon {
  ...
  const melee = sprite.startsWith("ml");
  const tier = r(1, 100);
  const rarity =
  ...
  const hue = r(2, 360);
  const name = melee
    ? `${PREFIX[r(3, PREFIX.length)]} ${MELEE_SUFFIX[r(4, MELEE_SUFFIX.length)]}`
    : `${PREFIX[r(3, PREFIX.length)]} ${SUFFIX[r(4, SUFFIX.length)]}`;
  const pellets = melee ? 3 + r(5, 3) : 1 + (r(5, 10) > 7 ? 1 + r(6, 4) : 0);
  ...
  return {
    ...
    sprite,
    rate: melee ? 0.24 + r(7, 30) / 100 : 0.09 + r(7, 40) / 100,
    damage: (melee ? 8 : 4) + power * 4 + r(8, 6),
    pellets,
    spread: melee ? 0.4 + r(9, 20) / 100 : 0.01 + r(9, 12) / 100,
    speed: melee ? 480 + r(2, 120) : 780 + r(2, 400),
    bulletRadius: melee ? 7 + r(3, 4) : 4 + r(3, 4),
    pierce: melee ? 1 + r(4, 3) : r(4, 3),
    knock: melee ? 18 + r(5, 18) : 5 + r(5, 12),
    shake: 2 + power * 1.5,
    visual: melee ? "ring" : VISUALS[r(6, VISUALS.length)]!,
    rarity,
  };
}
...
function rollShopOffers(s: GameState, count = 4): WeaponKey[] {
  ...
  const pool = SHOP_POOL.filter((k) => !s.arsenal.includes(k));
  const src = pool.length >= count ? pool : SHOP_POOL.slice();
  const out: WeaponKey[] = [];
  const bag = src.slice();
...
function spawnEnemy(s: GameState, forceElite = false, opts: SpawnOpts = {}) {
  ...
  const x = opts.x ?? Math.cos(a) * dist;
  const y = opts.y ?? Math.sin(a) * dist;
...
export function update(s: GameState, input: Input, dt: number) {
  ...
  // fixed circular border, same size for the whole run (Brotato-style arena)
  ...
  const pad = 70;
  const limX = s.arenaR + pad - WORLD_W / 2;
  const limY = s.arenaR + pad - WORLD_H / 2;
...
export function render(ctx: CanvasRenderingContext2D, s: GameState, sprites: Sprites, time: number) {
  ...
  const drawEcho = (e: Echo) => {
    ...
    if (!e.dead) {
      drawGun(ctx, sprites, e.weapon, e.x, e.y - 44, e.aim, e.facing, 0, e.muzzle > 0);
      // echo hp bar
      ctx.globalAlpha = 0.8;
      ctx.fillStyle = "rgba(10,8,16,0.8)";
  ...
  if (!s.over) {
    ...
    drawGun(ctx, sprites, p.weapon, p.x, p.y - 46, p.aim, p.facing, recoil, s.muzzle > 0);
  }
  ctx.restore();
